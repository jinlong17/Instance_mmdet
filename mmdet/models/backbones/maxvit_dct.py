...
@MODELS.register_module(name='MaxViT_DCT', force=True)
class MaxViT_DCT(nn.Module):
    ...
    def __init__(
        ...
        """ Constructor method """
        # Call super constructor
        super(MaxViT_DCT, self).__init__()
        # Check parameters
        ...
        # window_size = [grid_window_size, tuple([int(grid_window_size[0]/2),int(grid_window_size[0]/2)]), tuple([int(grid_window_size[0]/4),int(grid_window_size[0]/4)]), tuple([int(grid_window_size[0]/5),int(grid_window_size[0]/5)])]
        # window_size = [tuple([16,16]), tuple([8,8]),tuple([4,4]),tuple([2,2])]
        window_size = [tuple([8,8]), tuple([4,4]),tuple([4,4]),tuple([2,2])]
        ...
        for index, (depth, channel) in enumerate(zip(depths, channels)):
            ...
            stages.append(
                MaxViTStage(
                    ...
                    num_heads=num_heads,
                    grid_window_size=grid_window_size,
                    # grid_window_size=window_size[index],
                    attn_drop=attn_drop,
                    drop=drop,
        ...
        self.stages_dct = nn.ModuleList(stages_dct)
        # Init convolutional stem
        in_channels_dct = 12
        self.stem_dct = nn.Sequential(
            nn.Conv2d(in_channels=in_channels_dct, out_channels=embed_dim, kernel_size=(3, 3), stride=(2, 2),
                      padding=(1, 1)),
            act_layer(),
            nn.Conv2d(in_channels=embed_dim, out_channels=embed_dim, kernel_size=(3, 3), stride=(1, 1),
                      padding=(1, 1)),
        ...
        self.cov_dct = nn.Sequential(
            nn.Conv2d(in_channels=12, out_channels=3, kernel_size=1),
            nn.BatchNorm2d(3,eps=1e-5, momentum=0.01, affine=True),
            nn.ReLU()
        ).cuda()
      
        self.cov_0 = nn.Sequential(
            nn.Conv2d(in_channels=self.channels[0], out_channels=self.channels[0], kernel_size=1),
            nn.BatchNorm2d(self.channels[0],eps=1e-5, momentum=0.01, affine=True),
            nn.ReLU()
        )
        self.cov_1 = nn.Sequential(
            nn.Conv2d(in_channels=self.channels[1], out_channels=self.channels[1], kernel_size=1),
            nn.BatchNorm2d(self.channels[1],eps=1e-5, momentum=0.01, affine=True),
            nn.ReLU()
        )
        self.cov_2 = nn.Sequential(
            nn.Conv2d(in_channels=self.channels[2], out_channels=self.channels[2], kernel_size=1),
            nn.BatchNorm2d(self.channels[2],eps=1e-5, momentum=0.01, affine=True),
            nn.ReLU()
        )
        self.cov_3 = nn.Sequential(
            nn.Conv2d(in_channels=self.channels[3], out_channels=self.channels[3], kernel_size=1),
            nn.BatchNorm2d(self.channels[3],eps=1e-5, momentum=0.01, affine=True),
            nn.ReLU()
        )
        self.conv_mdct = nn.ModuleList([self.cov_0, self.cov_1, self.cov_2, self.cov_3])
    ...
    def forward_multi_features(self, input: torch.Tensor) -> torch.Tensor:
        """ Forward pass of feature extraction.
    ...
    def feature_dct_15(self, input: torch.Tensor) -> torch.Tensor:

        
        out = []

        dct_f = input.clone()####DCT feature
        rgb_f = input#### RGB feature

       
        _, dct_f_list = self.DCT_process(dct_f)



        dct_f_list = torch.cat(dct_f_list, dim=1)
        combine = torch.cat([rgb_f, dct_f_list], dim=1)


        dct_f_list_f = self.stem(combine)

        for stage in self.stages:

            dct_f_list_f = stage(dct_f_list_f)

            out.append(dct_f_list_f)
        return out

    def feature_dct_multi_fusion(self, input: torch.Tensor) -> torch.Tensor:
        ...
        dct, dct_f_list = self.DCT_process(dct_f)
        # dct_f_list_f = torch.max(torch.cat(dct_f_list, dim=1), dim=1, keepdim=True)


        # dct_f = []
        # for feat in dct_f_list:
            
        #     dct_f.append(torch.max(feat, dim=1, keepdim=True)[0])
        
        # dct_f_max = torch.cat(dct_f,dim=1)

        # dct_f_max = dct_f_list[-1]

        # dct_f_max =  self.cov_dct(dct)
        # dct_f_list_f = self.stem(dct_f_max)

        dct_f_list_f = self.stem_dct(dct)
        rgb_f = self.stem(rgb_f)
        ...
        i=0
        for block in range(len(self.stages)):


            # rgb_f = self.stages[block](rgb_f)
            # dct_f_list_f = self.stages_dct[block](dct_f_list_f)
            # fusion = [rgb_f,dct_f_list_f]
            # # output = torch.max(torch.cat(fusion, dim=0), dim=0, keepdim=True)[0]
            # output = fusion[0]+fusion[1]
            # # output = self.conv_mdct[i](output)
            ...
            # fusion = [rgb_f,dct_f_list_f]
            # output = fusion[0]+fusion[1]
            # output = self.conv_mdct[block](output)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        ####baseline: mulit-scale features output
        # output = self.forward_multi_features(self.stem(input))


        ########## option 0: frequency 12+3--->15 as mulit-scale features output
        output = self.feature_dct_15(input)

        ####option 1: multi-scale fuse DCT feature and RGB feature
        # output = self.feature_dct_multi_fusion(input)
